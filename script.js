document.addEventListener('DOMContentLoaded', function() {
    ...
    const todayMsgCount = document.getElementById('today-msg-count');
    ...
    auth.onAuthStateChanged(user => {
        currentUser = user;
        if (user) {
            loginSection.style.display = 'none'; dashboardSection.style.display = 'block'; settingsSection.style.display = 'none';
            ...
            loginSection.style.display = 'block'; dashboardSection.style.display = 'none'; settingsSection.style.display = 'none';
            if (navLogout) navLogout.style.display = 'none'; if (chatInput) chatInput.disabled = true; if (chatSendBtn) chatSendBtn.disabled = true;
    ...
    async function loadDashboardData(email) {
        ...
        try {
            ...
            else { let html = ''; userSessions.forEach(doc => { const d = doc.data(); const t = d.timestamp ? d.timestamp.toDate().toLocaleString('ar-EG') : 'غير معروف'; html += `<li>🟢 دخول في: ${t}</li>`; }); loginHistory.innerHTML = html; if (sessionCount) sessionCount.textContent = userSessions.length; if (lastLogin) lastLogin.textContent = userSessions[0].data().timestamp.toDate().toLocaleString('ar-EG'); }
    ...
    if (exportBtn) {
        exportBtn.addEventListener('click', async () => {
            ...
            finally { exportBtn.disabled = false; exportBtn.textContent = '📥 تصدير الدردشة (CSV)'; }
        });
    }
    ...
    function initChat() {
        ...
        unsubscribeChat = db.collection('messages').orderBy('timestamp','asc').onSnapshot(snapshot => {
            ...
            if (snapshot.empty) messagesList.innerHTML = '<p style="text-align:center;color:#888;padding:20px;">🎉 لا توجد رسائل بعد. كن أول من يكتب!</p>';
                ...
                let hasNewMessages = false; cachedMessages = [];
                snapshot.forEach(doc => {
                    const m = doc.data(); cachedMessages.push({ id: doc.id, ...m });
                    ...
                    const div = document.createElement('div'); div.className = 'message-item' + (isMe ? ' my-message' : '');
                    ...
                    if (isMe || isAdmin) { const delBtn = document.createElement('button'); delBtn.className = 'msg-delete-btn'; delBtn.textContent = '🗑️'; delBtn.title = 'حذف الرسالة'; delBtn.onclick = async () => { if (confirm('هل تريد حذف هذه الرسالة نهائياً؟')) { try { await db.collection('messages').doc(doc.id).delete(); } catch (err) { alert('❌ فشل الحذف: ' + err.message); } } }; div.appendChild(delBtn); }
    ...
    if (chatSendBtn) {
        chatSendBtn.addEventListener('click', async () => {
            const text = chatInput?.value.trim(); if (!text || !currentUser) return;
